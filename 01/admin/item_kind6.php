...
?>
<div  class="right">
  <div class="right01"> 通路介紹-第2層分類管理</div>
...
<?	  
$sql="select * from tb_item_kind5 order by standing";
$rs2=mysql_query($sql);
$totnum= mysql_num_rows($rs2);  

if ($totnum<=0) {
   echo "<center>沒有任何通路介紹-第1層分類,<BR>無法新增通路介紹-第2層分類!</center>";
   exit;
}

$say="";	
?>
<form name="form2" method="post" action="item_kind6.php">
...
<?
if ($cid!=""){
$sql="select * from tb_item_kind6 where cid=$cid order by standing";
$rs=mysql_query($sql);
$totnum= mysql_num_rows($rs);  

if ($totnum<=0) {
   echo "<BR><BR><center>沒有任何通路介紹-第2層分類資料...<input type=button value=\"新增通路介紹-第2層分類\" onclick=\"location.replace('item_kind6_add.php?cid=$cid')\"></center>";
}else{
?>
<input type=button value="新增第2層分類" onclick="location.replace('item_kind6_add.php?cid=<?=$cid;?>')"><BR><BR>
</td></tr>
</table>
...
?>
         <tr>
             ...
             <td align=center><a href="item_kind6_edit.php?cid=<?=$cid;?>&ccid=<?=$row["ccid"];?>">修改</a></td>
             <td align=center><input type=text name="standing_<?=$iii;?>" value="<?=$row["standing"];?>" style="width:40px;"></td>
<td align=center>
<input type=button value="上移" onclick="location.replace('item_kind6_standing.php?cid=<?=$cid;?>&ccid=<?=$row["ccid"];?>&kind=1')">
<input type=button value="下移" onclick="location.replace('item_kind6_standing.php?cid=<?=$cid;?>&ccid=<?=$row["ccid"];?>&kind=2')">
<input type=button value="置頂" onclick="location.replace('item_kind6_standing.php?cid=<?=$cid;?>&ccid=<?=$row["ccid"];?>&kind=3')">
<input type=button value="最後" onclick="location.replace('item_kind6_standing.php?cid=<?=$cid;?>&ccid=<?=$row["ccid"];?>&kind=4')">
</td>
             </tr>
<?
  }  
...
document.form1.action="item_kind6_standing2.php?cid=<?=$cid;?>&cidstr=<?=$cidstr;?>";
...
function delcid(){
         ...
         if (! confirm("是否確定刪除?")){
            ...
            location.href="item_kind6_del.php?cid=<?=$cid;?>&cidstr="+cidstr;
         }
}
</script>
<? }
