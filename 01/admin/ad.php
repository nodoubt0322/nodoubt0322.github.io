...
?>
<div  class="right">
  <div class="right01"> 左下方廣告(寬:191)</div>
...
<?	  
if ($totnum<=0) {
   echo "<center>沒有任何左下方廣告(寬:191)資料...<input type=button value=\"新增左下方廣告(寬:191)\" onclick=\"location.replace('ad_add.php')\"></center>";
}else{
?>


<input type=button value="新增左下方廣告(寬:191)" onclick="location.replace('ad_add.php')">
<BR><BR>
...
<td align=center width="40%">標題</td>
...
?>
         <tr height=30 onMouseOver="gbg(this)" onMouseOut="gbn(this)">
			 ...
			 <td align="center">
			  ...
			  <img src="../pic/sdfsxfv/<?=$row["pic"];?>" width=190 border=0></img>
			 ...
			 <td align=center><a href="ad_edit.php?aid=<?=$row["aid"];?>">修改</a></td>
             <td align=center><input type=text name="standing_<?=$iii;?>" id="standing_<?=$iii;?>" value="<?=$row["standing"];?>" style="width:40px;"></td>
			<td align=center>
			<input type=button value="上移" onclick="location.replace('ad_standing.php?aid=<?=$row["aid"];?>&kind=1')">
			<input type=button value="下移" onclick="location.replace('ad_standing.php?aid=<?=$row["aid"];?>&kind=2')">
			<input type=button value="置頂" onclick="location.replace('ad_standing.php?aid=<?=$row["aid"];?>&kind=3')">
			<input type=button value="最後" onclick="location.replace('ad_standing.php?aid=<?=$row["aid"];?>&kind=4')">
			</td>
             </tr>
<?
  }  
...
document.forms['form1'].action="ad_standing2.php";
...
function delaid(){		 
         ...
         if (! confirm("是否確定刪除?")){
            ...
            location.href="ad_del.php?aidstr="+aidstr;
         }
}
</script>
<? } ?>
