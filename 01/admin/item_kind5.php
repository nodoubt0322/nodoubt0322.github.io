...
?>
<div  class="right">
  <div class="right01"> 通路介紹-第1層分類管理</div>
...
<?	  
if ($totnum<=0) {
   echo "<center>沒有任何通路介紹-第1層分類資料...<input type=button value=\"新增分類\" onclick=\"location.replace('item_kind5_add.php')\"></center>";
}else{
?>


<input type=button value="新增分類" onclick="location.replace('item_kind5_add.php')">
<BR><BR>
...
?>
         <tr height=30 onMouseOver="gbg(this)" onMouseOut="gbn(this)">
			 ...
			 <td align=center><a href="item_kind5_edit.php?cid=<?=$row["cid"];?>">修改</a></td>
             <td align=center><input type=text name="standing_<?=$iii;?>" id="standing_<?=$iii;?>" value="<?=$row["standing"];?>" style="width:40px;"></td>
<td align=center>
<input type=button value="上移" onclick="location.replace('item_kind5_standing.php?cid=<?=$row["cid"];?>&kind=1')">
<input type=button value="下移" onclick="location.replace('item_kind5_standing.php?cid=<?=$row["cid"];?>&kind=2')">
<input type=button value="置頂" onclick="location.replace('item_kind5_standing.php?cid=<?=$row["cid"];?>&kind=3')">
<input type=button value="最後" onclick="location.replace('item_kind5_standing.php?cid=<?=$row["cid"];?>&kind=4')">
</td>
             </tr>
<?
  }  
...
document.forms['form1'].action="item_kind5_standing2.php";
...
function delcid(){		 
         ...
         if (! confirm("是否確定刪除?")){
            ...
            location.href="item_kind5_del.php?cidstr="+cidstr;
         }
}
</script>
<? } ?>
