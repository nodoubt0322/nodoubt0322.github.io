...
if(is_uploaded_file($_FILES['myfile']['tmp_name'])){
	...
	if ($w!=980)
	{
	    echo "寬必須為980px....<a href=\"javascript:history.go(-1);\">回上一頁</a>";
	    exit;
	}
	if ($h!=400)
	{
	    echo "高必須為400px....<a href=\"javascript:history.go(-1);\">回上一頁</a>";
	    exit;
	}
	if (is_file("../pic/main_banner/".$old_pic)) {
       	    unlink("../pic/main_banner/".$old_pic);
        }
}
...
$sql="update tb_main_banner set subject='$subject_ct',pic='$ServerFilename',url='$url',openkind='$openkind' where aid=$aid";
mysql_query($sql);

				echo "<script language=javascript>
				     alert (\"修改成功. \");
					 document.location.href=\"main_banner.php\";
					 </script>";
?>